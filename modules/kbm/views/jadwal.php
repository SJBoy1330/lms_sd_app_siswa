<!-- Begin page -->
<main class="h-100 py-0">

    <!-- main page content -->
    <div class="main-container container top-20">
        <div class="row mt-3">
            <div class="col-12 col-md-10 col-lg-8 mx-auto">
                <div class="row mb-3 px-1">
                    <div class="col align-self-center">
                        <h5 class="title">Jadwal Pelajaran</h5>
                    </div>
                    <div class="col-auto align-self-center">
                        <select class="form-select form-select-pribadi" aria-label="Default select example">
                            <option selected>Senin</option>
                            <option value="1">Selasa</option>
                            <option value="2">Rabu</option>
                            <option value="3">Kamis</option>
                            <option value="4">Jum'at</option>
                        </select>
                    </div>
                </div>
                <div class="row">
                    <?php if (isset($result)) : ?>
                        <?php foreach ($result as $row) : ?>
                            <div class="col-12 mx-auto">
                                <div class="card mb-4">
                                    <div class="card-body">
                                        <div class="row mb-3">
                                            <div class="col-auto">
                                                <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                                    <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                        <i class="fa-brands fa-stack-overflow size-24 text-white"></i>
                                                    </div>
                                                </div>
                                            </div>
                                            <?php if ($row->kbm == 'Y') : ?>
                                                <div class="col align-self-center ps-0">
                                                    <p class="mb-0 size-12 fw-normal text-secondary">Mata Pelajaran</p>
                                                    <p class="mb-0 size-13 fw-medium"><?= $row->nama_pelajaran; ?></p>
                                                </div>
                                            <?php else : ?>
                                                <div class="col align-self-center ps-0">
                                                    <p class="mb-0 size-12 fw-normal text-secondary">Nama Kegiatan</p>
                                                    <p class="mb-0 size-13 fw-medium"><?= $row->kegiatan_lain; ?></p>
                                                </div>
                                            <?php endif; ?>
                                        </div>

                                        <div class="row mb-3">
                                            <div class="col-auto">
                                                <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                                    <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                        <i class="fa-solid fa-clock size-24 text-white"></i>
                                                    </div>
                                                </div>
                                            </div>
                                            <div class="col-auto align-self-center ps-0">
                                                <p class="mb-0 size-12 fw-normal text-secondary">Jam Pelajaran</p>
                                                <p class="mb-0 size-13 fw-medium"><?= substr($row->jam_mulai, 0, -3) ?> - <?= substr($row->jam_selesai, 0, -3) ?></p>
                                            </div>
                                        </div>
                                        <?php if ($row->kbm == 'Y') : ?>
                                            <div class="row">
                                                <div class="col-auto">
                                                    <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                                        <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                            <i class="fa-solid fa-chalkboard-user size-20 text-white"></i>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div class="col align-self-center ps-0">
                                                    <p class="mb-0 size-12 fw-normal text-secondary">Nama Guru</p>
                                                    <p class="mb-0 size-13 fw-medium"><?= $row->nama_staf; ?></p>
                                                </div>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
                    <?php endif; ?>

                    <div class="col-12 mx-auto">
                        <div class="card mb-4">
                            <div class="card-body">
                                <div class="row mb-3">
                                    <div class="col-auto">
                                        <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                            <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                <i class="fa-brands fa-stack-overflow size-24 text-white"></i>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col align-self-center ps-0">
                                        <p class="mb-0 size-12 fw-normal text-secondary">Mata Pelajaran</p>
                                        <p class="mb-0 size-13 fw-medium">Matematika</p>
                                    </div>
                                </div>

                                <div class="row mb-3">
                                    <div class="col-auto">
                                        <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                            <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                <i class="fa-solid fa-clock size-24 text-white"></i>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-auto align-self-center ps-0">
                                        <p class="mb-0 size-12 fw-normal text-secondary">Jam Pelajaran</p>
                                        <p class="mb-0 size-13 fw-medium">08:00 - 10:00</p>
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-auto">
                                        <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                            <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                <i class="fa-solid fa-chalkboard-user size-20 text-white"></i>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col align-self-center ps-0">
                                        <p class="mb-0 size-12 fw-normal text-secondary">Nama Guru</p>
                                        <p class="mb-0 size-13 fw-medium">Drs. Astutik S.Pd</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>

                    <div class="col-12 mx-auto">
                        <div class="card mb-4">
                            <div class="card-body">
                                <div class="row mb-3">
                                    <div class="col-auto">
                                        <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                            <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                <i class="fa-brands fa-stack-overflow size-24 text-white"></i>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col align-self-center ps-0">
                                        <p class="mb-0 size-12 fw-normal text-secondary">Mata Pelajaran</p>
                                        <p class="mb-0 size-13 fw-medium">Pendidikan Jasmani, Olahraga, dan Kesehatan </p>
                                    </div>
                                </div>

                                <div class="row mb-3">
                                    <div class="col-auto">
                                        <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                            <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                <i class="fa-solid fa-clock size-24 text-white"></i>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col-auto align-self-center ps-0">
                                        <p class="mb-0 size-12 fw-normal text-secondary">Jam Pelajaran</p>
                                        <p class="mb-0 size-13 fw-medium">10:00 - 11:00</p>
                                    </div>
                                </div>

                                <div class="row">
                                    <div class="col-auto">
                                        <div class="avatar avatar-50 shadow-sm rounded-12 avatar-presensi-outline">
                                            <div class="avatar avatar-40 rounded-10 avatar-presensi-inline">
                                                <i class="fa-solid fa-chalkboard-user size-20 text-white"></i>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="col align-self-center ps-0">
                                        <p class="mb-0 size-12 fw-normal text-secondary">Nama Guru</p>
                                        <p class="mb-0 size-13 fw-medium">Pak Raditya Chandra</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- main page content ends -->


</main>
<!-- Page ends-->